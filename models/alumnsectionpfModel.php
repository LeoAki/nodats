<?php
/**
 * Description of alumnsectionpfModel
 *
 * @author aquino
 */
class alumnsectionpfModel extends Model {
    ...
    public function grabar() {
        $this->_db->query('
        Call SP_Alumno_Seccion_Pf(\''.$this->CODIGO.'\',\''.$this->CODIGOAS.'\',\''.$this->UPF1.'\',\''.$this->UPF2.'\',\''.$this->UPF3.'\',
    ...
    public function calificacionppffbycodealumno($codeals) {#poniedole su codigo de alumno seccion
        $codeals = (int) $codeals;
        $list=$this->_db->query('
	...
	INNER JOIN Seccion s ON ase.idseccion = s.codigo
        where ase.idalumnoseccion='.$codeals);
        return $list->fetchall();
    }

    public function calificacionppffbynamealumno($paterno,$materno,$nombres) {#poniedole su codigo de alumno seccion
        $list=$this->_db->query('
        SELECT ase.idalumnoseccion, ase.nroorden, p.paterno, p.materno, p.nombres,
          asep.1pf1,asep.1pf2,asep.1pf3,asep.1pf4,asep.1pf5,asep.1pf6,asep.1pf7,asep.1pf8,
          asep.2pf1,asep.2pf2,asep.2pf3,asep.2pf4,asep.2pf5,asep.2pf6,asep.2pf7,asep.2pf8,
	...
	INNER JOIN Seccion s ON ase.idseccion = s.codigo
        where
        p.paterno like(\'%'.$paterno.'%\') and
        p.materno like(\'%'.$materno.'%\') and
        p.nombres like(\'%'.$nombres.'%\')');
        return $list->fetchall();
    }

    public function alumnosdemitutoria($codedocente) {
        $codedocente = (int) $codedocente;
        $list=$this->_db->query('
        SELECT ase.idalumnoseccion, ase.nroorden, p.paterno, p.materno, p.nombres,
          asep.1pf1,asep.1pf2,asep.1pf3,asep.1pf4,asep.1pf5,asep.1pf6,asep.1pf7,asep.1pf8,
          asep.2pf1,asep.2pf2,asep.2pf3,asep.2pf4,asep.2pf5,asep.2pf6,asep.2pf7,asep.2pf8,
